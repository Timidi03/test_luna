...
1. Для просмотра Swagger UI или Redoc в браузере переходим по ссылке `http://127.0.0.1:8000/docs` или `http://127.0.0.1:8000/redoc` соответсвенно
2. API ключ хранится в переменных среды в фалйе `docker-compose.yaml` в сервисе `app`. При неоходимости его можно заменить.
3. Ключ передается в заголовке `X-API-Key` (в Swagger достаточно нажать на странице кнопку Authorize и ввести значение в поле)
